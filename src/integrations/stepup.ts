...
export function setupStepUpIntegration(): void {
  ...
  async function handleSetFilesAsync(data: { files: Array<{ path: string; content: string; language?: string }> }) {
    ...
    for (const [id] of runtime.fileModels) disposeModel(id);
    const activeTab = await tabManager.replaceAllFiles(data.files, runtime.currentLang!, runtime.currentVersion!);
    if (activeTab) {
      ...
      if (activeFileLang) {
        ...
        runtime.currentVersion = populateVersionDropdown(activeFileLang, activeTab.file.version);
        configureMonacoForVersion(activeFileLang, runtime.currentVersion);
  ...
  async function handleInit(data: any) {
    ...
    if (Array.isArray(data.files) && data.files.length > 0 && appConfig.ideMode !== 'snippet') {
      ...
      if (appConfig.isEmbedded) {
        ...
        const tab = await tabManager.replaceAllFiles(
          [{ path: fileName, content: data.code, language: runtime.currentLang!.id }],
          runtime.currentLang!, runtime.currentVersion!
        );
        if (tab) {
          ...
          const model = monaco.editor.getModel(uri) || monaco.editor.createModel(data.code, runtime.currentLang!.id, uri);
          model.setValue(data.code);
          editor.setModel(model);
          updateEmptyState(false);
        }
    ...
    if (data.autoRun && policyState.allowRun) setTimeout(() => runBtn.click(), 200);
  }
  ...
  window.addEventListener('message', event => {
    ...
    switch (type) {
      case 'stepup:init': void handleInit(data); break;
      case 'stepup:set-code': if (typeof data.code === 'string') editor.setValue(data.code); break;
      case 'stepup:get-code': sendToParent('ide:code-response', { code: editor.getValue(), language: appConfig.urlLanguage, version: appConfig.urlVersion }); break;
      case 'stepup:set-files': void handleSetFilesAsync(data); break;
      case 'stepup:get-files': {
        if (appConfig.ideMode === 'snippet') {
          sendToParent('ide:files', { files: [{ path: 'main', content: editor.getValue(), language: appConfig.urlLanguage }] });
        } else {
          void collectWorkspaceSnapshot().then(files => {
            if (!files.length) files.push({ path: 'main', content: editor.getValue(), language: appConfig.urlLanguage });
            sendToParent('ide:files', { files });
          });
        }
        break;
      }
      case 'stepup:run': if (policyState.allowRun) runBtn.click(); break;
      case 'stepup:set-readonly': applyPolicyFromMessage(data); break;
      case 'stepup:show-output': {
        clearTurtleCanvas();
        let text = typeof data.output === 'string' ? data.output : '';
        ...
        setOutput(text);
        if (data.turtleData?.shapes?.length) renderTurtle(data.turtleData);
        break;
      }
      case 'stepup:clear-output': setOutput(''); clearTurtleCanvas(); break;
    }
  });

  let filesSnapshotTimeout: ReturnType<typeof setTimeout> | null = null;
  runtime.notifyWorkspaceChanged = () => {
    if (!appConfig.isEmbedded || appConfig.ideMode === 'snippet' || policyState.readonly) return;
    if (filesSnapshotTimeout) clearTimeout(filesSnapshotTimeout);
    filesSnapshotTimeout = setTimeout(async () => sendToParent('ide:files', { files: await collectWorkspaceSnapshot() }), 500);
  };

  let codeChangeTimeout: ReturnType<typeof setTimeout> | null = null;
  editor.onDidChangeModelContent(() => {
    const activeTab = tabManager.getActiveTab();
    if (activeTab) tabManager.markDirty(activeTab.file.id, editor.getValue());
    if (appConfig.isEmbedded && !policyState.readonly) {
      if (codeChangeTimeout) clearTimeout(codeChangeTimeout);
      codeChangeTimeout = setTimeout(() => notifyCodeChange(editor.getValue()), 300);
      runtime.notifyWorkspaceChanged();
    }
  });
  ...
  if (appConfig.isEmbedded) {
    ...
    if (initialOrigin) setParentOrigin(initialOrigin);
    notifyParentReady(policyState.readonly);
