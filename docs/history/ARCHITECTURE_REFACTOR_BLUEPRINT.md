...
## 48. PHP, through Xdebug over DBGp

### 48.1 The assumption that was wrong

Section 47.7 recorded PHP debugging as blocked because "Xdebug is not in the image", and
that was a guess dressed as a fact. Docker Desktop turned out to be installed on the
development host, so the question could be settled by asking Alpine instead:

```
$ docker run --rm node:20-alpine sh -c 'apk update -q && apk search -q xdebug'
php-pecl-xdebug
php83-pecl-xdebug
php84-pecl-xdebug
php85-pecl-xdebug
...
One package. Everything in this section follows from a check that took ninety seconds
and should have been run before the item was written off.

### 48.2 What it costs an ordinary run: nothing

The obvious objection to installing a debugger into the runtime image is that every PHP
program then pays for it - Xdebug's overhead on a normal script is real and large.

It does not, and the reason is in the package. Alpine ships
`/etc/php84/conf.d/50_xdebug.ini` with the loader line **commented out**:
...
Verified in a container: `php -m` does not list it and `extension_loaded("xdebug")` is
false. The debug adapter turns it on for its own process only, with
`-dzend_extension=xdebug`. An ordinary run is byte-for-byte the run it was before.

### 48.3 The one structural difference from every other adapter here

**Xdebug dials out.** The debugger listens on a port, PHP is told that port at startup,
and the engine connects back. There is nothing to attach to.

That inverts the Java adapter, which spawns a JVM and then attaches to it, and it
decides the shape of the PHP one: bind first, spawn second, wait. It is also why the
debug launch starts a Node process rather than the interpreter - something has to be
listening before PHP exists.

### 48.4 DBGp, briefly

XML over a socket. Each message from the engine is `<length in ASCII> NUL <XML> NUL`;
each command to the engine is one NUL-terminated line.

The property worth naming is that **there is no event channel**. Unlike JDWP, nothing
arrives unprompted: `run` does not return until the program next stops, and the stop is
that command's own reply, carrying `status="break"` and an `<xdebug:message>` with the
file and line. So there is no interleaving to handle - and one rule that must not be
broken, because breaking it corrupts the session rather than delaying it: **exactly one
command may be outstanding**. Both the session and the adapter's command handler
serialise for that reason.

### 48.5 Why the XML parser is written here

DBGp XML is machine-generated by one implementation: elements, attributes, CDATA, no
namespaces to resolve, no DTDs, no mixed content that matters. That is a hundred lines.
A general-purpose XML parser would add a dependency to the set of files copied into the
runtime image and loaded next to a student's program - the same trade this project
already refused for source maps and for JDWP.

The subtlety that makes arrays work is that **CDATA belongs to the element containing
it, not to its parent**. A `<property>` holding a string has CDATA of its own; a
`<property>` holding an array has child `<property>` elements, each with theirs.
Merging text upward would give an array the concatenation of its children's values,
which is not an error - it is a plausible-looking wrong answer in the variables panel.

### 48.6 Two protocol bugs, each pinned by a test

| What went wrong | Why |
| --- | --- |
| Every `eval` came back "invalid or missing options" | `--` means "the rest of this line is the base64 payload", so `eval -- <data> -i 3` makes the transaction id part of the data. `-i` has to come first |
| The PHP process never exited and the run hung until the session timer killed it | Xdebug answers the final `run` with `status="stopping"`, not `"stopped"`: the script has finished but the engine is still holding the process open waiting for one more command. The student saw their output, and then nothing |

### 48.7 A test that proved nothing, and what replaced it

The contract suite had a test asserting that a breakpoint on `../../etc/passwd` is
refused rather than armed. It passed. It also passed with the adapter's containment
check **deleted** - measured, not assumed - because PHP's own `open_basedir` is set to
the same directory and refuses the file first.

A test that cannot fail is worse than no test: it reads as coverage. The two path
helpers therefore moved out of `debug_adapter.mjs` into `dbgp.mjs`, and the reason is
testability rather than tidiness - the adapter opens a socket the moment it is imported,
so a unit test cannot load it at all, and anything living there can only ever be
exercised through the end-to-end suite.

In `dbgp.mjs` they are provable with no PHP on the machine, including the case neither
the adapter nor `open_basedir` would have caught by string comparison alone: a sibling
directory whose name starts with the workspace's (`/jobs/abc-2` against `/jobs/abc`).
The contract test keeps its assertion, now labelled as the end-to-end outcome it
actually is.

### 48.8 Watch expressions are real here

Unlike Java, where evaluating an expression would mean shipping a compiler front end,
`eval` hands the expression to the engine and it runs as PHP. `count($list)`,
`strtoupper($name)` and `$person->name` all work.

That is more capable and also sharper: the expression is arbitrary code with the
program's own permissions. It is only ever sent while the program is paused, and the
interpreter's `disable_functions` list still applies to anything it calls - the debug
launch is given the same hardening flags as an ordinary run, so `open_basedir` and
`disable_functions` are identical under the debugger. A debug run must not be looser
than a normal one, or students find out which one is.

`disable_functions` includes `eval`, and DBGp's `eval` still works: it is implemented by
the engine rather than by PHP's `eval()` construct. That is asserted by a contract test
rather than assumed, because it is exactly the kind of thing that could quietly change.

### 48.9 Verified

**In a container built from the same base as the runtime image** (`node:20-alpine` plus
`php php-pecl-xdebug`, PHP 8.4.21, Xdebug 3.5.0): 13 contract tests over real debug
sessions - two files, a cross-file stack, locals including an uninitialised one,
function-calling watches, a step, and a clean exit - and 26 unit tests on the wire
format. Both of the first two bugs in 48.6 were re-introduced and the suite confirmed to
fail before the fix was restored.
